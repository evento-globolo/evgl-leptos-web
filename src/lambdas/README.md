...
This nested crate proves that heavy and infrequent routes from `evento-globolo/evgl-leptos-web` can be built and deployed independently of the main Leptos web server. It does not change the existing server entrypoint, initialize Leptos, open a listener, create a WebSocket, or construct the main application/database state.

## Entry points

| Binary | API Gateway route | Required body field | Intended workload |
|---|---|---|---|
| `heavy_badge_render` | `POST /api/heavy/badge-render` | `attendee_id` | Render a badge without starting Leptos SSR. |
| `heavy_venue_plan` | `POST /api/heavy/venue-plan` | `event_id` | Generate an expensive venue plan independently. |
